...
class Excel(object):
    ...
    def write_line(self,row_data,row_no = None,background_color=None):
        ...
        if not row_no:
            ...
            for col_num in range(len(row_data)):
                ...
                if row_data[col_num]=="失败" or "fail" in row_data[col_num] or "失" in row_data[col_num] or  "败" in row_data[col_num]:
    ...
    def write_cell_current_time(self,row_no,col_no):
        date = str(time.localtime().tm_year)+"年"+str(time.localtime().tm_mon)+"月"+str(time.localtime().tm_mday)+"日"
        now_time = str(time.localtime().tm_hour)+"时"+str(time.localtime().tm_min)+"分"+str(time.localtime().tm_sec)+"秒"
        now = date+" "+now_time
